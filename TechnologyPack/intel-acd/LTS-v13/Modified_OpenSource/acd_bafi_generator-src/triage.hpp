                  ...
                  json silkscreenMap)
{
  ...
  if ((summary.cpuType == "SPR" || summary.cpuType == "SPR-HBM" || summary.cpuType == "EMR") && optionalCpuId)
  {
  ...
  {
    TORDataIcxMap tor;
    cpu->analyze(inputJson, tor);
    Report<TORDataIcxMap> report(summary, tor, summary.cpuType, cpu->cpuId, deviceMap,
                         silkscreenMap, inputJson);
    // Generate JSON report from collected data
    reportJson = report.createJSONReport();
  }
  else if (summary.cpuType == "CPX" || summary.cpuType == "SKX")
  {
    TORDataCpxSkxMap tor;
    cpu->analyze(inputJson, tor);
    Report<TORDataCpxSkxMap> report(summary, tor, summary.cpuType, cpu->cpuId, deviceMap,
                         silkscreenMap, inputJson);
    // Generate JSON report from collected data
    reportJson = report.createJSONReport();
  }
  else if (summary.cpuType == "SPR" || summary.cpuType == "SPR-HBM" || summary.cpuType == "EMR")
  {
    TORDataSprMap tor;
    cpu->analyze(inputJson, tor);
    Report<TORDataSprMap> report(summary, tor, summary.cpuType, cpu->cpuId, deviceMap,
                         silkscreenMap, inputJson);
    // Generate JSON report from collected data
    reportJson = report.createJSONReport();
  }
...
void getErrorStrings(std::string entry, std::string &ierr, std::string &mcerr,
                     std::string &rmcaMcerr, std::string &sidebandPortId, std::string cpuType)
{
  ...
  if (cpuType == "SPR" || cpuType == "SPR-HBM" || cpuType == "EMR")
  {
    ...
    right = std::string(entry)
    .substr(left, std::string(entry).size()).find(", SIDEBANDPORTID");
    rmcaMcerr = std::string(entry)
    .substr(left, right).substr(20);
  }
  else
  {
    right = std::string(entry)
...
{
  getErrorStrings(entry, ierr, mcerr, rmcaMcerr, sidebandPortId, cpuType);
...
void getMemoryErrors(json reportJson, std::string socket,
                     nlohmann::ordered_json &triageReport, nlohmann::ordered_json &dimmEntry)
{
...
void getThermalErrors(std::string socket, nlohmann::ordered_json &triageReport,
                      nlohmann::ordered_json &cpuThermalEntry, Summary& summary)
{
...
void getChaErrors(std::string socket, std::string mcerr, std::string sidebandPortId, nlohmann::json reportJson, nlohmann::ordered_json &triageReport)
{
  for (const auto& [pciKey, pciVal] : reportJson["MCA"][socket].items())
  {
    for (const auto& [mcaKey, mcaVal] : pciVal.items())
    {
      if (mcaKey == "Bank" && std::string(mcaVal).find("CHA") != std::string::npos &&
         (mcerr.find(mcaVal) != std::string::npos || sidebandPortId.find(mcaVal) != std::string::npos))
      {
        std::string bdfStr = "";
    ...
    for (const auto& [pciKey, pciVal] : reportJson["output_summary"][socket].items())
    {
      if (pciVal.contains("Errors_Per_Socket"))
      {
        for (const auto& [metaKey, metaVal] : pciVal.items())
        {
          ...
          {
            firstError =
                determineFirstErrorType(std::string(tscVal), ierr, mcerr, rmcaMcerr, sidebandPortId, summary.cpuType);
            if (firstErrorSocket == "")
            {
              firstError =
                determineFirstErrorType(std::string(tscVal), ierr, mcerr, rmcaMcerr, sidebandPortId, summary.cpuType);
            ...
            {
              getErrorStrings(std::string(tscVal), ierr, mcerr, rmcaMcerr, sidebandPortId, summary.cpuType);
            }
        ...
        for (const auto& [metaKey, metaVal] : pciVal.items())
        {
          for (const auto& [tscKey, tscVal] : metaVal.items())
          {
            ...
            if (std::string(tscKey).find("IMC") != std::string::npos)
            {
              ...
              imcEntry[socket].push_back(tscKey);
            }
            ...
            if (std::string(tscKey).find("UPI") != std::string::npos)
            {
              ...
              cpuUpiEntry["UPI_Errors"][socket].push_back(tscKey);
            }
          }
        }
      }
