...
class SprMcaDecoder : public McaDecoder
{
  ...
  protected:
    ...
    const std::map<uint8_t, const char*> ChannelSpr = {
    ...
    const std::map<uint8_t, const char*> ChannelSprHbm = {
        {0x0, "Channel_0"},  {0x1, "Channel_1"},
        {0x2, "Channel_2"},  {0x3, "Channel_3"},
        {0x4, "Channel_4"},  {0x5, "Channel_5"},
        {0x6, "Channel_6"},  {0x7, "Channel_7"},
        ...
        {0xa, "Channel_10"}, {0xb, "Channel_11"},
        {0xc, "Channel_12"}, {0xd, "Channel_13"},
        {0xe, "HBM"}, {0xf, "Channel_not_specified"},
    };
    ...
    std::string decodeMcacodGeneric2(uint16_t mcacod, bool isHbm = false)
    {
        std::stringstream ss;
        ...
        auto channel =
            getDecoded(ChannelSpr, static_cast<uint8_t>(mcacodDecoded.channel));
        if (isHbm)
        {
            channel = getDecoded(ChannelSprHbm, static_cast<uint8_t>(mcacodDecoded.channel));
        }
        auto error =
...
class SprMcaBankIfu final : public SprMcaDecoder
{
  private:
    ...
    {
        uint32_t msmcacod = status_decoded.mc_status & 0xFFFF07FF;

        auto mscodDecoded = getDecoded(decodeMscod,
            static_cast<uint32_t>(msmcacod));
        if (mscodDecoded)
        {
...
class SprMcaBankDcu final : public SprMcaDecoder
{
  private:
    ...
    const std::map<uint8_t, const char*> decodeMscod2 = {
        {0x0, "Non-APIC Error"},
    ...
    void decodeKnownFields()
    {
      auto mscodDecoded = getDecoded(
          decodeMscod, static_cast<uint32_t>(status_decoded.mc_status));
      if (mscodDecoded)
      {
          entry["MSCOD_decoded"] = *mscodDecoded;
      }

      auto mscod2Decoded = getDecoded(
          decodeMscod2, static_cast<uint8_t>(status_decoded.mscod));
      if (mscod2Decoded && !mscodDecoded)
      {
          entry["MSCOD_decoded"] = *mscod2Decoded;
      }
...
private:
    ...
    {
      auto mscodDecoded = getDecoded(decodeMscod,
          static_cast<uint32_t>(status_decoded.mc_status));
      if (mscodDecoded)
      {
          entry["MSCOD_decoded"] = *mscodDecoded;
      }
  ...
  class SprMcaBankMlc final : public SprMcaDecoder
{
    ...
    void decodeKnownFields()
    {
        uint32_t msmcacod = status_decoded.mc_status;
        ...
        auto mscodDecoded = getDecoded(
            decodeMscod, static_cast<uint32_t>(msmcacod));
        if (mscodDecoded)
        {
            entry["MSCOD_decoded"] = *mscodDecoded;
        }
...
class SprMcaBankPcu final : public SprMcaDecoder
{
    ...
    const std::map<uint32_t, const char*> decodeMscodShutdownError = {
        {0x1040c, "Shutdown Error: MCE when CR4.MCE is clear"},
        {0x2040c, "Shutdown Error: MCE when MCIP bit is set"},
        {0x3040c, "Shutdown Error: MCE under WPS"},
        {0x4040c, "Shutdown Error: Unrecoverable error during security flow execution"},
        {0x5040c, "Shutdown Error: SW triple fault shutdown"},
        {0x7040c, "Shutdown Error: RSM consistency check failures"},
        {0x8040c, "Shutdown Error: Invalid conditions on protected mode SMM entry"},
        {0x9040c, "Shutdown Error: Unrecoverable error during security flow execution"},
        {0x6040c, "Shutdown Error: VMX exit consistency check failures"}
    };
    ...
    void decodeKnownFields()
    {
        ...
        // If shutdown error, else decode as usual
        if ((status_decoded.mc_status & 0xffff) == 0x40c)
        {
            auto mscod1916Decoded = getDecoded(
            decodeMscodShutdownError, static_cast<uint32_t>(status_decoded.mc_status));
            if (mscod1916Decoded)
            {
...
class SprMcaBankCha final : public SprMcaDecoder
{
    ...
    const std::map<uint8_t, const char*> rawChaDecoding9 = {
        ...
        {0xE, "Most severe Error ID = RAW CHA42"},
        {0xF, "Most severe Error ID = RAW CHA45"},
        {0x10, "Most severe Error ID = RAW CHA48"},
        {0x11, "Most severe Error ID = RAW CHA51"},
        {0x12, "Most severe Error ID = RAW CHA54"},
        {0x13, "Most severe Error ID = RAW CHA57"},
        {0x14, "Most severe Error ID = RAW CHA60"},
    };
    ...
    const std::map<uint8_t, const char*> rawChaDecoding10 = {
        ...
        {0xD, "Most severe Error ID = RAW CHA40"},
        {0xE, "Most severe Error ID = RAW CHA43"},
        {0xF, "Most severe Error ID = RAW CHA46"},
        {0x10, "Most severe Error ID = RAW CHA49"},
        {0x11, "Most severe Error ID = RAW CHA52"},
        {0x12, "Most severe Error ID = RAW CHA55"},
        {0x13, "Most severe Error ID = RAW CHA58"},
    };
    ...
    const std::map<uint8_t, const char*> rawChaDecoding11 = {
        ...
        {0xE, "Most severe Error ID = RAW CHA44"},
        {0xF, "Most severe Error ID = RAW CHA47"},
        {0x10, "Most severe Error ID = RAW CHA50"},
        {0x11, "Most severe Error ID = RAW CHA53"},
        {0x12, "Most severe Error ID = RAW CHA56"},
        {0x13, "Most severe Error ID = RAW CHA59"},
    };
    union MC_STATUS
    {
         struct
...
class SprHbmMcaBankM2m final : public SprMcaDecoder
{
  private:
    union MC_STATUS
    {
      struct
      {
          uint64_t mcacod: 16, mscod: 16, other_info: 5, fw_update: 1,
...
class SprHbmMcaBankImc final : public SprMcaDecoder
{
    ...
    void decodeKnownFields()
    {
        ...
        entry["MCACOD_decoded"] = decodeMcacodGeneric2(status_decoded.mcacod, true);
    }
