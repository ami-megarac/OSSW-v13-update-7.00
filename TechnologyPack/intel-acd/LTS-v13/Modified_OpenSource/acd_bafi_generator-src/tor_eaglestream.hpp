...
class EaglestreamCpu
{
    ...
    getMemoryMap(const json& input)
    {
        ...
        for (auto const& [cpu, cpuSection] : cpuSections)
        {
            ...
            if  (cpuSection.get().find("uncore") == cpuSection.get().cend())
            {
                continue;
            }

            for (auto const& [name, value] : cpuSection.get()["uncore"].items())
            {
                ...
                if (name.find(mmcfgBase) != std::string::npos)
                {
                    auto shortName = "MMCFG";
                    memoryMap[shortName][0] = uintValue;
                }
                else if (name.find(mmcfgLimit) != std::string::npos)
                {
                    auto shortName = "MMCFG";
    ...
    [[nodiscard]] std::optional<std::string>
        getMetaDataInfo(const json& input, std::string socket,
                       std::string varName)
    {
        if (input.contains("METADATA"))
        {
            if (input["METADATA"].find(socket) != input["METADATA"].cend())
            {
                if (input["METADATA"][socket].find(varName) !=
                    input["METADATA"][socket].cend())
                {
                    if (input["METADATA"][socket][varName] .is_string() &&
                        !checkInproperValue(input["METADATA"][socket][varName]))
                    {
