...
class GenericReport
{
  ...
  protected:
            ...
            json& summaryEntry)
    {
        ...
        auto sideBandPortId = getDecoded(firstErrorTable,
            static_cast<uint16_t>(ctx.ncEventsCrSmiSrcLogIcx.sideBandPortId));
        auto sideBandPortValid = ctx.ncEventsCrSmiSrcLogIcx.sideBandValid;
        if (Cpu == "SPR" || Cpu == "SPR-HBM" || Cpu == "EMR")
        {
            ...
            sideBandPortId = getDecoded(firstErrorTable,
                static_cast<uint16_t>(ctx.ncEventsCrSmiSrcLogSpr.sideBandPortId));
            sideBandPortValid = ctx.ncEventsCrSmiSrcLogSpr.sideBandValid;
        }
        else
        {
