...
class IcxCpu final : public WhitleyCpu, public CpuGeneric
{
  public:
    ...
    [[nodiscard]] std::vector<TORDataIcx> getTorsData(const json &input)
    {
        std::vector<TORDataIcx> torsData;
    ...
    void analyze(const json &input, TORDataIcxMap &output)
    {
        ...
        for (auto const& [cpu, cpuSection] : cpuSections)
        {
            ...
            std::vector<TORDataIcx> tors = getTorsData(cpuSection);
            uint32_t socketId;
            if (!str2uint(cpu.substr(3), socketId, decimal))
            {
                continue;
            ...
            if (!ncevents_cr_smisrclog || !str2uint(*ncevents_cr_smisrclog, ctx.ncEventsCrSmiSrcLogIcx.value))
            {
                ctx.ncEventsCrSmiSrcLogIcx.value = 0;
            }
            ...
            std::pair<SocketCtx, std::vector<TORDataIcx>>
                tempData(ctx, tors);
